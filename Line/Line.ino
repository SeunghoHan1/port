...
float Kp =60;// Max deviation = 4-2.5 = 1.5 ||  255/1.5 = 72 기준
float Ki = 0.20; // 정확도 상승 높으면 시간이 오래걸림 낮으면 거의변화없음
float Kd = 22;// 빠르게 목표치까지 접근, 높으면 Overshoot 상승
...
void PID_program()
{
  if(digitalRead(8)==0&&digitalRead(9)==0&&digitalRead(10)==0&&digitalRead(11)==0) // IR 센서가 라인을 완전히 벗어난 경우
  ...
  else // 라인 위에 있을 경우 PID 실행 
    {
      ...
      totalError += error;
      power = (Kp*error) + (Kd*(error-previousError)) + (Ki*totalError); // power 0~255
